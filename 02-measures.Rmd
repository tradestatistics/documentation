...
We smoothed changes in export volumes induced by the price fluctuation of products by using a modification of \@ref(eq:rca) in which $x_{c,p}$ is averaged over the previous three years by using weights:
...
Consider that for some years this needs to be altered. As an example, for 1962 the SRCA is the same as RCA and for 1963 the $x_{c,p}^{(t-3)}$ part is omitted and the denominator is changed to 3.

## Country-product matrix

From \@ref(eq:srca) we constructed a matrix that connects each country to the products that it exports. The entries in the matrix are 1 if country $c$ exports product $p$ with Smooth Revealed Comparative Advantage larger than 1, and 0 otherwise. The matrix $S \in \mathbb{R}^{C\times P}$ has entries defined as:
...
\begin{equation}
(\#eq:kcn2)
k_{c}^{(n)} = \frac{1}{k_{c}^{(0)}} \sum_p s_{c,p} \left(\frac{1}{k_{p}^{(0)}} \sum_c s_{c,p} k_{c}^{(n-2)}\right)
\end{equation}

\@ref(eq:kcn2) above can be conveniently written as a matrix equation (this formulation takes some ideas from [@mesuringcomplexity2015] and [@interpretationreflections2014]):
...
In particular, the interpretation of the scores changes when considering odd or even iteration order $n$, high-order iterations are difficult to interpret, and the process asymptotically converges to a trivial fixed point according to [@mesuringcomplexity2015].
